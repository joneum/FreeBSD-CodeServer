...
Open VSX registry like on any other platform. Some popular extensions bundle
or download **native Linux binaries** (language servers, debug adapters) that
do not run on FreeBSD. The fix is always the same two-step recipe:
...
the extension installs, the setting keys are taken from the extensions' own
manifests, and the binary paths from the ports' packing lists.
...
| Python | `ms-python.python` | works out of the box (Jedi) | for type checking add `ms-pyright.pyright` (pure JS, no setup) |
...
| Ruby | `shopify.ruby-lsp` | `pkg install ruby` + `gem install ruby-lsp` | `"rubyLsp.rubyExecutablePath": "/usr/local/bin/ruby"` |
...
| Python debugging (`ms-python.debugpy`) | installs, but bundles Linux-only native wheels; launch (F5) may fail | `pkg install py312-debugpy`, start your program with `python3 -m debugpy --listen 5678 --wait-for-client prog.py` and use a "Remote Attach" launch configuration |
